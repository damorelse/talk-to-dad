...
import { SyllableCard } from './SyllableCard.js';
...
export const SyllableVisualizerView = ({ categories: propCategories, cards: propCards, }) => {
    ...
    const { word, setWord, speed, pronunciationData, activeSyllableIdx, isPlaying: isEnPlaying, playSingleSyllable, soundItOut: soundItOutEn, speakWholeWord: speakWholeWordEn, stop: stopEn, } = usePiperSyllables('Water', 0.5);
    useEffect(() => {
    ...
    const filteredVocabList = useMemo(() => {
        ...
        if (searchQuery.trim()) {
            ...
            list = list.filter((item) => item.wordEn.toLowerCase().includes(q) ||
    ...
    const currentSelectedCard = useMemo(() => {
        if (isCustomActive || !selectedItemId)
            return null;
    ...
    const handleSelectVocabItem = (item) => {
        stopAll();
    ...
    const handleSelectVocabMode = (mode) => {
        if (mode === vocabMode)
            return;
        stopAll();
        setVocabMode(mode);
    ...
    const playSingleChineseSyllable = (idx) => {
        const chars = Array.from(word).filter((c) => !/[\s，。！？、,!?.]/.test(c));
        if (!chars[idx])
            return;
        setZhActiveIdx(idx);
    ...
    const chineseSyllables = isCurrentChinese
        ? Array.from(word)
    ...
    return (_jsxs("div", { className: "w-full h-full flex flex-col gap-1.5 sm:gap-2 overflow-y-auto p-1 sm:p-2 select-none scrollbar-thin", children: [_jsxs("div", { className: "w-full flex flex-col sm:flex-row items-center justify-between gap-2 bg-transparent border border-transparent px-1 py-0.5 sm:py-1 rounded-2xl shrink-0", children: [_jsxs("div", { className: "flex items-center gap-2 flex-wrap w-full sm:w-auto", children: [_jsxs("div", { className: "flex items-center gap-1 bg-slate-950 p-1 rounded-xl border border-slate-800", children: [_jsx(Languages, { className: "w-3.5 h-3.5 text-slate-400 ml-1.5 mr-0.5" }), _jsxs("button", { type: "button", onClick: () => handleSelectLanguage('en'), className: `
              ...
              `, children: [_jsx("span", { children: "English" }), language === 'en' && (_jsx("span", { className: "w-1.5 h-1.5 rounded-full bg-emerald-500 animate-pulse shrink-0", title: "Piper Neural Speech Active" }))] }), _jsxs("button", { type: "button", onClick: () => handleSelectLanguage('zh'), className: `
                px-3 py-1 rounded-lg text-xs font-bold transition-all cursor-pointer flex items-center gap-1.5
                ${language === 'zh' ? 'bg-amber-400 text-slate-950 font-black shadow-xs' : 'text-slate-400 hover:text-slate-200'}
              `, children: [_jsx("span", { children: "\u4E2D\u6587" }), language === 'zh' && (_jsx("span", { className: "text-[10px] font-bold text-slate-950 px-1 bg-amber-300 rounded-sm leading-none", title: "\u6CE8\u97F3\u7B26\u865F (BoPoMoFo)", children: "\u6CE8" }))] })] }), _jsxs("div", { className: "flex items-center gap-1 bg-slate-950 p-1 rounded-xl border border-slate-800", children: [_jsx("button", { type: "button", onClick: () => handleSelectVocabMode('words'), className: `
                px-2.5 py-1 rounded-lg text-xs font-bold transition-all cursor-pointer
                ${vocabMode === 'words' ? 'bg-amber-400 text-slate-950 font-black shadow-xs' : 'text-slate-400 hover:text-slate-200'}
              `, children: "Phrase" }), _jsx("button", { type: "button", onClick: () => handleSelectVocabMode('phrases'), className: `
                px-2.5 py-1 rounded-lg text-xs font-bold transition-all cursor-pointer
                ${vocabMode === 'phrases' ? 'bg-amber-400 text-slate-950 font-black shadow-xs' : 'text-slate-400 hover:text-slate-200'}
              `, children: "Sentence" })] })] }), _jsxs("form", { onSubmit: handleApplyCustom, className: "flex items-center gap-1.5 w-full sm:w-72 md:w-80 shrink-0", children: [_jsx("input", { type: "text", value: customInput, onChange: (e) => setCustomInput(e.target.value), placeholder: "Type custom word...", className: "flex-1 px-3 py-1.5 bg-slate-950 border border-slate-700 rounded-xl text-xs sm:text-sm text-white placeholder-slate-500 focus:outline-none focus:ring-2 focus:ring-amber-400" }), _jsx("button", { type: "submit", disabled: !customInput.trim(), className: "px-3.5 py-1.5 bg-amber-400 hover:bg-amber-300 text-slate-950 text-xs sm:text-sm font-bold rounded-xl disabled:opacity-50 transition-all cursor-pointer whitespace-nowrap", children: "Analyze" })] })] }), _jsxs("div", { className: "w-full shrink-0 min-h-0 h-auto bg-transparent border border-transparent rounded-3xl pt-0.5 sm:pt-1 pb-3 sm:pb-4 px-4 sm:px-6 md:px-8 flex flex-col items-center justify-center gap-1.5 sm:gap-2 relative transition-all duration-200", children: [_jsx("div", { className: "flex items-center gap-2.5 px-3 py-0 rounded-2xl bg-transparent border border-transparent text-sm sm:text-base font-bold text-slate-300 shrink-0", children: currentSelectedCard ? (_jsxs(_Fragment, { children: [currentSelectedCard.icon && _jsx("span", { className: "text-lg sm:text-2xl leading-none", children: currentSelectedCard.icon }), _jsx("span", { className: "text-white font-black", children: currentSelectedCard.wordEn }), _jsx("span", { className: "text-slate-500", children: "\u00B7" }), _jsx("span", { className: "text-amber-300 font-bold", children: currentSelectedCard.wordZh })] })) : (_jsxs(_Fragment, { children: [_jsx("span", { className: "text-amber-400", children: "\u270F\uFE0F" }), _jsx("span", { className: "text-white font-black", children: word }), _jsx("span", { className: "text-slate-400 text-xs", children: "(Custom)" })] })) }), _jsx("div", { className: "w-full max-w-5xl py-0.5 sm:py-1 flex items-center justify-center", children: _jsx(SyllableCard, { word: word, syllableData: isCurrentChinese ? chineseSyllables : pronunciationData.syllables, activeSyllableIndex: currentActiveIdx, onSyllableClick: (_, idx) => {
                                if (isCurrentChinese) {
                            ...
                            }, showIpa: isCurrentChinese, showStress: !isCurrentChinese }) }), _jsxs("div", { className: "flex items-center gap-3.5 sm:gap-4 flex-wrap justify-center pt-0 sm:pt-0.5 shrink-0", children: [hasMultipleParts && (_jsxs(DebouncedTouchable, { onPress: () => {
                                    if (isCurrentChinese) {
                                        soundItOutChinese();
                                    }
                                    else {
                                ...
                                }, disabled: isPlaying, minTouchSize: "lg", className: `
              ...
              `, children: [_jsx(MessageSquareQuote, { className: "w-6 h-6 stroke-[2.5]" }), _jsx("span", { children: "Sound It Out" })] })), _jsxs(DebouncedTouchable, { onPress: () => {
                                    if (isCurrentChinese) {
                  ...
                  `, children: [item.icon && _jsx("span", { className: "text-base leading-none shrink-0", children: item.icon }), _jsx("span", { className: isSelected ? 'text-slate-950 font-black' : 'text-white', children: item.wordEn }), _jsxs("span", { className: `text-[11px] ${isSelected ? 'text-slate-900 font-bold' : 'text-slate-400'}`, children: ["\u00B7 ", item.wordZh] }), item.isWeeklyTherapy && (_jsx("span", { title: "Weekly Therapy Focus", className: "text-amber-400 text-xs ml-0.5", children: "\u2728" }))] }, item.id));
                        })) })] })] }));
};
