import { formatWithMiddleDot } from '../syllables/syllableSplitter.js';
import { isChineseText } from '../audio/WebSpeechEngine.js';
const VALID_FITZGERALD_ROLES = new Set([
...
export class GoogleSheetsService {
     ...
     */
    parseGoogleSheetUrl(urlOrId, sheetName, gidOverride) {
     ...
     */
    async fetchGoogleSheetCsv(urlOrId, options = {}) {
        ...
        try {
            ...
            if (text.includes('<!DOCTYPE html>') || text.includes('<html')) {
                if (text.includes('accounts.google.com') || text.includes('ServiceLogin')) {
                    throw new Error('The Google Sheet is private and requires Google Login. Please set sharing permissions to ' +
     ...
     */
    parseCardsData(rows, headers, categories, defaultCategoryId) {
        const errors = [];
        const cards = [];
        ...
        for (let r = 0; r < rows.length; r++) {
            ...
            if (roleIdx !== -1 && row[roleIdx]) {
                ...
                if (VALID_FITZGERALD_ROLES.has(rawRole)) {
                    role = rawRole;
                }
            ...
            if (catIdx !== -1 && row[catIdx]) {
                ...
                const matched = categories.find(c => c.id.toLowerCase() === catStr ||
     ...
     */
    parseSheetData(csvText, options) {
