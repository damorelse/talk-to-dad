...
import { phonemizeWord } from '../services/syllables/espeakPhonemizer.js';
import { piperTTSService } from '../services/syllables/piperTTSService.js';
import { piperOnnxService } from '../services/syllables/piperOnnxService.js';
import { iosAudioUnlock } from '../services/audio/iOSAudioUnlock.js';
import { speechEngine } from '../services/audio/WebSpeechEngine.js';
export function usePiperSyllables(initialWord = 'Photography', initialSpeed = 0.5) {
    const [word, setWordState] = useState(initialWord);
    ...
    const [pronunciationData, setPronunciationData] = useState(() => phonemizeWord(initialWord));
    ...
    const setWord = useCallback((newWord) => {
    ...
    useEffect(() => {
        ...
        const warmCache = async () => {
            if (!pronunciationData || pronunciationData.syllables.length === 0)
                return;
    ...
    const playSingleSyllable = useCallback(async (index) => {
    ...
    const soundItOut = useCallback(async () => {
        if (isPlaying || pronunciationData.syllables.length === 0)
            return;
        setIsPlaying(true);
        setActiveSyllableIdx(null);
        iosAudioUnlock.ensureUnlockedAndResumed();
