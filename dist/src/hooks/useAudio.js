...
import { isChineseText } from '../services/audio/WebSpeechEngine.js';
...
export function useAudio() {
     ...
     */
    const speakBilingual = useCallback(async (enText, zhText, audioBlobId, customOptions) => {
        setIsSpeaking(true);
        ...
        try {
            if (mode === 'zh') {
                ...
                const locale = zhText ? 'zh-TW' : 'en-US';
                ...
                const firstLocale = zhText ? 'zh-TW' : 'en-US';
    ...
    const speakCard = useCallback((card, customOptions) => {
        return speakBilingual(card.spokenText || card.label, card.spokenTextZh || card.labelZh, card.audioBlobId, customOptions);
    }, [speakBilingual]);
    const speakHotspot = useCallback((hotspot, customOptions) => {
        return speakBilingual(hotspot.spokenText || hotspot.label, hotspot.spokenTextZh || hotspot.labelZh, hotspot.audioBlobId, customOptions);
    }, [speakBilingual]);
    const speakPainReport = useCallback((enSentence, zhSentence, customOptions) => {
        return speakBilingual(enSentence, zhSentence, undefined, customOptions);
    }, [speakBilingual]);
    const triggerEmergency = useCallback(async (enPhrase, zhPhrase) => {
        setIsSpeaking(true);
        try {
