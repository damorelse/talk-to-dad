 ...
 */
import { phonemizeWord, getCanonicalIpa, formatIpaWithSyllables } from './espeakPhonemizer.js';
import { piperTTSService, generateDeterministicPhonemeAudio } from './piperTTSService.js';
...
export const CLINICAL_SYLLABLE_DICTIONARY = {
 ...
 */
export function algorithmicSyllableSplit(word) {
 ...
 */
function matchCase(syllables, original) {
    let charIdx = 0;
...
export function splitWordIntoSyllables(word) {
    ...
    if (CANONICAL_PHONEME_DICTIONARY[lower]) {
        return matchCase(CANONICAL_PHONEME_DICTIONARY[lower].syllables.map((s) => s.text), clean);
    }
 ...
 */
export function formatWithMiddleDot(text) {
    if (!text)
        return '';
 ...
 */
export function breakPhraseForVisualizer(phrase) {
    if (!phrase)
        return [];
 ...
 */
export function getWordPronunciation(word) {
    return phonemizeWord(word);
}
/**
 * Returns syllable breakdown array with IPA data.
 */
export function getSyllableBreakdownWithIpa(word) {
    const data = phonemizeWord(word);
    return data.syllables;
}
// Re-exports
export { phonemizeWord, getCanonicalIpa, formatIpaWithSyllables, piperTTSService, generateDeterministicPhonemeAudio, CANONICAL_PHONEME_DICTIONARY, };
