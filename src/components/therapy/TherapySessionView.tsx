...
export const TherapySessionView: React.FC<TherapySessionViewProps> = ({
  ...
  const [selectedCategoryId, setSelectedCategoryId] = useState<string | 'favorites'>('favorites');
  ...
  return (
    <div className="w-full h-full flex flex-col gap-2.5 overflow-hidden select-none">
      {/* Category Selector Tabs with Right-Aligned Distinct Reset Button */}
        ...
        <div className="flex-1 w-full flex flex-col items-center justify-center my-auto min-h-0 py-0.5 sm:py-1">
          {currentCard ? (
            <div className="w-full max-w-lg flex flex-col gap-1.5 sm:gap-2">
              {/* Practice Stats Bar: Score (Left) | Progress Counter (Right) - Lowered right above the card */}
              <div className="w-full flex items-center justify-between px-2 shrink-0">
                ...
                <div className="flex items-center gap-1.5 text-slate-300 font-bold text-xs sm:text-sm">
                  <span>{weeklyCards.length > 0 ? cardIndex + 1 : 0} of {weeklyCards.length}</span>
                </div>
              </div>
            ...
            <div className="flex flex-col items-center justify-center text-slate-400 gap-2 text-center p-4">
              ...
              <p className="text-base font-semibold text-white">No favorite cards yet.</p>
              <p className="text-xs text-slate-400">Star cards in the Cards view to practice them in Word Finding!</p>
            </div>
          ) : (
            <div className="flex flex-col items-center justify-center text-slate-400 gap-2">
              <span className="text-4xl">📭</span>
              <p className="text-base font-semibold">No cards available in this category.</p>
            </div>
          )}
        </div>
        ...
        <div className="w-full max-w-lg flex flex-col gap-2 shrink-0 pb-0.5">
          ...
          <div className="flex items-center justify-between gap-2.5 w-full">
            ...
            <DebouncedTouchable
              ...
              disabled={!currentCard}
              minTouchSize="md"
              className="flex-[1.4] bg-emerald-600 hover:bg-emerald-500 active:bg-emerald-700 text-white py-2.5 sm:py-3 rounded-xl sm:rounded-2xl font-black flex items-center justify-center gap-1.5 text-sm sm:text-base shadow-lg shadow-emerald-900/40 disabled:opacity-40 border-2 border-emerald-400"
            ...
            <DebouncedTouchable
              onPress={handleNext}
              disabled={cardIndex >= weeklyCards.length - 1 || !currentCard}
              minTouchSize="md"
              className="flex-1 bg-slate-800 hover:bg-slate-700 text-slate-200 py-2.5 sm:py-3 rounded-xl sm:rounded-2xl border border-slate-700 flex items-center justify-center gap-1.5 text-sm sm:text-base font-bold disabled:opacity-40 shadow-xs"
            >
              <span>Next</span>
