...
export const TIMEZONE_LOCATION_MAP: Record<
  string,
  { city: string; state?: string; country: string; cityZh?: string; stateZh?: string; countryZh?: string; lat: number; lon: number }
> = {
 ...
 */
export function getFallbackLocationFromTimezone(): UserLocationInfo {
  try {
 ...
 */
async function reverseGeocodeCoords(lat: number, lon: number): Promise<Partial<UserLocationInfo> | null> {
...
export async function detectUserLocation(): Promise<UserLocationInfo> {
  ...
  return new Promise<UserLocationInfo>((resolve) => {
...
// -----------------------------------------------------------------------------
// CLINICAL SPEECH FORMATTERS
// -----------------------------------------------------------------------------

export interface WeekdayDef {
  index: number; // 0 = Sunday, 1 = Monday, ..., 6 = Saturday
  name: string;
  nameShort: string;
  nameZh: string;
  nameZhShort: string;
...
export function getDayPeriod(hours: number): { en: string; zh: string; icon: string } {
  if (hours >= 5 && hours < 12) {
 ...
 */
export function getWeekDates(currentDate: Date): WeekCalendarDay[] {
 ...
 */
export function formatWeekdaySpeech(date: Date): { en: string; zh: string } {
 ...
 */
export function formatLocationSpeech(loc: UserLocationInfo): { en: string; zh: string } {
  const locationPartsEn: string[] = [];
  if (loc.city) locationPartsEn.push(loc.city);
  if (loc.state) locationPartsEn.push(loc.state);
  if (loc.country) locationPartsEn.push(loc.country);

  const enStr = locationPartsEn.join(', ');

  let zhStr = '';
  if (loc.countryZh || loc.country) zhStr += (loc.countryZh || loc.country);
  if (loc.stateZh || loc.state) zhStr += (loc.stateZh || loc.state);
