...
import { splitWordIntoSyllables } from './syllableSplitter.ts';
 ...
 */
export function phonemizeGraphemeSyllable(
  syllableText: string,
  isPrimaryStressed: boolean,
  isSecondaryStressed: boolean,
  isFinalSyllable: boolean,
  _totalSyllables?: number
): { ipa: string; phonemes: string[] } {
  ...
  while (i < clean.length) {
    ...
    switch (char) {
      ...
      default:
        ...
        if (char.trim()) phonemes.push(char);
        break;
    }
    i++;
  }
 ...
 */
export function calculateStressIndices(syllables: string[], fullWord: string): { primaryIdx: number; secondaryIdx: number | null } {
  const count = syllables.length;
  if (count <= 1) return { primaryIdx: 0, secondaryIdx: null };
 ...
 */
export function phonemizeWord(word: string): WordPronunciationData {
  if (!word || !word.trim()) {
    return {
      word: '',
      canonicalIpa: '',
  ...
  if (CANONICAL_PHONEME_DICTIONARY[lower]) {
    ...
    const syllables: SyllablePhonemeData[] = entry.syllables.map((s, idx) => ({
      index: idx,
      text: s.text,
      ipa: s.ipa,
      phoneticSpelling: s.phoneticSpelling || s.text,
      stress: s.stress as SyllableStress,
      phonemes: [...s.phonemes],
    }));
  ...
  const syllables: SyllablePhonemeData[] = textSyllables.map((sylText, idx) => {
    const isPrimary = idx === primaryIdx;
    const isSecondary = idx === secondaryIdx;
    const stress: SyllableStress = isPrimary ? 'primary' : isSecondary ? 'secondary' : 'unstressed';

    const { ipa, phonemes } = phonemizeGraphemeSyllable(
      sylText,
      isPrimary,
      isSecondary,
      idx === textSyllables.length - 1,
      textSyllables.length
    );
 ...
 */
export function getCanonicalIpa(word: string): string {
 ...
 */
export function formatIpaWithSyllables(word: string): string {
  const data = phonemizeWord(word);
