...
let ortInstance: any = typeof globalThis !== 'undefined' && (globalThis as any).ort 
...
async function getOrt(): Promise<any> {
  ...
  try {
    ...
    return (globalThis as any)?.ort || (window as any)?.ort || null;
  }
}
...
class PiperOnnxService {
  ...
  private loadStatus: ModelLoadStatus = 'unloaded';
  ...
  getStatus(): { status: ModelLoadStatus; progress: number; error: string | null } {
    return {
  ...
  async loadModel(onProgress?: (percent: number) => void): Promise<void> {
    if (this.loadStatus === 'ready' && this.session) return;
    ...
    try {
      ...
      if (!configRes.ok) throw new Error(`Failed to load config: HTTP ${configRes.status}`);
      this.config = (await configRes.json()) as PiperConfig;
      ...
      if (!modelBuffer) {
        ...
        if (!response.ok) throw new Error(`Failed to load model binary: HTTP ${response.status}`);
   ...
   */
  phonemesToIds(phonemes: string[]): number[] {
    if (!this.config) throw new Error('Piper configuration not loaded');
